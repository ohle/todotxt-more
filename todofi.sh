...
MARKUP_LINENO='<small><small><span fgcolor="black">\1<\/span><\/small><\/small>'
...
MARKUP_ISSUE='<small><span fgcolor="blue"><b>\1<\/b><\/span><\/small>'
MARKUP_CREATED='<small><span fgcolor="gray"><b>\1<\/b><\/span><\/small>'
...
highlight() {
    ...
    echo "${line}" | sed -r "
        ...
        s/(issue:https?:\/\/[^[:blank:]]*)/${MARKUP_ISSUE}/g;
        s/((created|updated):[^[:blank:]]*)/${MARKUP_CREATED}/g;
        s/(https?:\/\/github.com\/)/gh:/g;
        s/\/issues\/([[:digit:]]+)/#\1/g;
...
option() {
    ...
    do
        if [[ ${current_line:0:1} == 'x' ]]; then
            ...
            if [ "$TODO_TIMETRACKER" != "0" ]; then
                extra="0. Track"
            else
                extra=""
            fi
            selection=$(echo -e "${extra}1. Mark Done|2. Edit|3. Edit priority|4. Remove priority|5. Delete|6. Open issue" | runrofi -lines 6 -sep "|" -kb-accept-entry "Return" -mesg "Item: ${current_line}" -dmenu -p "Action")
            lineno=$(getlinenumber "$current_line")

            case "${selection:0:1}" in
              "0")
                timetrack "$current_line" && break;;
              "1")
...
Todo.txt format: https://github.com/todotxt/todo.txt"
    ...
    selection=$(
        ...
        runrofi -sep "|" -lines 6 -kb-accept-entry "Return" -mesg "${HELP}" -dmenu -p "Action"
    )
    val=$?
